      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden pb-6">
        ...
        <div className="px-10 pb-4 flex flex-col md:flex-row justify-between items-start md:items-center">
          <div className="flex items-center gap-5">
            ...
            <div className="mt-2">
              <h2 className="m-0 text-xl text-gray-800 font-semibold">{formData.nome}</h2>
              <a href={`mailto:${formData.email}`} className="text-sm text-gray-500 hover:underline">
                {formData.email}
              </a>
            </div>
          </div>
          ...
          <button 
            ...
            className={`mt-4 md:mt-2 text-white border-none px-6 py-2 rounded-md text-sm font-medium cursor-pointer transition-colors ${
              isEditing ? "bg-blue-600 hover:bg-blue-700" : "bg-[#316f27] hover:bg-green-800"
            }`}
          >
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-5 px-10 pt-6">
          <div className="flex flex-col gap-2">
            ...
            <input type="text" name="nome" value={formData.nome} onChange={handleChange} readOnly={!isEditing} className={inputClassName} />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-gray-600 font-medium">Email</label>
            <input type="email" name="email" value={formData.email} onChange={handleChange} readOnly={!isEditing} className={inputClassName} />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-gray-600 font-medium">Nome de usuário</label>
            <input type="text" name="username" value={formData.username} onChange={handleChange} readOnly={!isEditing} className={inputClassName} />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-gray-600 font-medium">Patente</label>
            <input type="text" name="patente" value={formData.patente} readOnly={true} className="px-4 py-3 rounded-md text-sm text-gray-500 bg-[#f6f6f6] border border-transparent outline-none cursor-not-allowed w-full" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-gray-600 font-medium">Nova Senha</label>
            <input type="password" name="senha" placeholder={isEditing ? "Digite uma nova senha" : "••••••••"} value={formData.senha} onChange={handleChange} readOnly={!isEditing} className={inputClassName} />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-gray-600 font-medium">Confirmar Nova Senha</label>
            <input type="password" name="confirmarSenha" placeholder={isEditing ? "Confirme a nova senha" : "••••••••"} value={formData.confirmarSenha} onChange={handleChange} readOnly={!isEditing} className={inputClassName} />
          </div>
        </form>
        ...
        <div className="px-10 mt-8 pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-4 justify-end">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <button type="button" className="bg-[#de3f53] hover:bg-[#c43648] text-white px-8 py-2.5 rounded-lg text-sm font-medium transition-colors">
                Sair
              </button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  A sua sessão será encerrada e precisará de fazer login novamente para aceder à arena.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancelar</AlertDialogCancel>
                <AlertDialogAction onClick={handleLogout} className="bg-[#316f27] hover:bg-green-800">
                  Sim, sair
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  e removerá seus dados dos nossos servidores de torneio.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancelar</AlertDialogCancel>
