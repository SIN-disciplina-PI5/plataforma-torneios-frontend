  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-380px rounded-2xl">
        ...
        {state === "confirm-cancel" && (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-sm text-gray-500 mt-1">
                Tem certeza que deseja se desinscrever de:
              </DialogDescription>
            </DialogHeader>
            <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 my-2">
              <p className="font-semibold text-gray-900 text-sm">{tournament?.nome}</p>
              <p className="text-xs text-gray-500 mt-0.5">
            ...
            <DialogFooter className="flex gap-2 mt-2">
              <Button variant="outline" onClick={onClose} className="flex-1">Voltar</Button>
        ...
        {/* LOADING */}
        {(state === "loading" || state === "cancel-loading") && (
          <>
            <VisuallyHidden.Root>
            ...
            <div className="flex flex-col items-center justify-center py-8 gap-4">
              ...
              <p className="text-sm text-gray-500">
                {state === "cancel-loading" ? "Cancelando inscrição..." : "Realizando inscrição..."}
              </p>
            </div>
          </>
        )}
        ...
        {state === "cancel-success" && (
          <>
            ...
            <div className="flex flex-col items-center justify-center py-6 gap-3">
              <CheckCircle2 className="text-gray-400" size={48} />
              <p className="text-sm text-gray-500 text-center">
                Sua inscrição em{" "}
