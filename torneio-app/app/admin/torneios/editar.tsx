...
export default function EditarTorneio() {
  ...
  async function editarTorneio() {
    ...
    try {
      ...
      console.log("Enviando atualização (PATCH):", payload)
      ...
      console.log("ERRO AO EDITAR:", error.response?.status, error.response?.data)
      ...
      if (status === 404) msgFinal = "Torneio não encontrado. Verifique se a URL está correta."
      if (status === 403) msgFinal = "Sem permissão. Verifique seu login de admin."
      if (msgBackend) msgFinal = msgBackend
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.centerContainer}>
          ...
          <Text style={styles.carregandoTexto}>Buscando informações...</Text>
        </View>
        <BarraNavegacaoAdmin />
      </SafeAreaView>
    )
