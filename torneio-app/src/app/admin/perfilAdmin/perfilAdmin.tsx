...
export default function AdminPerfil() {
    ...
    async function carregarPerfil() {
        try {
            ...
            const idEncontrado = dados.id || dados._id || dados.id_usuario || dados.userId;

            if (idEncontrado) {
                setId(idEncontrado);
                console.log("✅ ID Identificado:", idEncontrado);
            } else {
                console.error("❌ ERRO: ID não encontrado no objeto de dados.");
                Alert.alert("Erro Técnico", "Não foi possível identificar seu usuário.");
            }
    ...
    async function atualizarPerfil() {
        console.log("🟢 1. Botão 'Salvar' foi pressionado!");
        console.log("📝 Dados atuais no estado:", { nome, email, id });

        if (!nome || !email) {
            console.log("🔴 Erro: Nome ou Email vazios");
        ...
        if (!id) {
            ...
            Alert.alert("Erro", "Usuário não identificado. Relogue no app.");
            return;
        }
        ...
        try {
            console.log("🟡 2. Iniciando requisição para o Back-end...");
            setSaving(true);
            const token = await AsyncStorage.getItem("@token");
            ...
            console.log("✅ 3. Sucesso! Resposta:", response.data);
            Alert.alert("Sucesso", "Perfil atualizado! ✅");
            setPassword(""); 
            
        } catch (error: any) {
            console.log("❌ 4. Erro no Catch:", error);
